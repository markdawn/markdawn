...
void CPositionBmp::Draw(CDC* pDC, WPARAM wparam, LPARAM lparam)
{
	...
	CString str(_T("FRONT WALL"));
	memDC.TextOut(rClientRect.Width()/2-20,20,str);
	memDC.TextOut(rClientRect.Width()/2-20,rClientRect.Height()-40,_T("BACK WALL"));
...
void CPositionBmp::GetSensorPoint(POINT* pPoint, CRect rRect)
{
	...
	pPoint[2].x = rRect.left+iRealWidth;
	pPoint[2].y = rRect.top;
	pPoint[6].x = rRect.left+3*iRealWidth;
	pPoint[6].y = rRect.top;

	pPoint[4].x = rRect.right;
	pPoint[4].y = rRect.top + iRealHight;
	pPoint[5].x = rRect.right;
	pPoint[5].y = rRect.top + 3*iRealHight;

	pPoint[7].x = rRect.left+3*iRealWidth;
	pPoint[7].y = rRect.bottom;
	pPoint[3].x = rRect.left+iRealWidth;
	pPoint[3].y = rRect.bottom;

	pPoint[1].x = rRect.left;
	pPoint[1].y = rRect.top + 3 * iRealHight;
